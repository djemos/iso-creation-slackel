...
answer="$(eval dialog \
	--title \"Enter Slackel version\" \
	--stdout \
	--inputbox \
	\"Enter the slackel version you want to create the iso for.\n\You can add suffixes like alpha1, beta1, RC1 etc here.:\" \
	0 0 )"
retval=$?
...
mkisofs -o ../slackel${LIBDIRSUFFIX}-${edition}-${ver}.iso \
  -R -J -A "Slackel${LIBDIRSUFFIX} Install" \
  -V "Slackel${LIBDIRSUFFIX} $edition $ver" \
  -hide-rr-moved \
  -v -d -N \
...
cd ..
isohybrid $ISOHYBRID_EFI_OPTS slackel${LIBDIRSUFFIX}-${edition}-${ver}.iso
